...
- **Files:** bytes only through `storage.py` (`path_for`, `receive_upload`, `remove`); serve with `files.file_response` so the inline allowlist and CSP always apply. Async routes only for streaming bodies; wrap DB calls in `run_in_threadpool` there.
