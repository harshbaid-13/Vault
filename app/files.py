"""Files: upload, list, download/view, rename, favorite, delete (S6). Folders arrive in S7.
...
def list_files(conn) -> list[dict]:
    """Newest first; rowid breaks ties in upload order (ids are random). S7 adds folders and sorting."""
    return [to_file(row) for row in conn.execute(f"SELECT {COLUMNS} FROM files ORDER BY created_at DESC, rowid DESC")]
...
@router.get("/files")
def files_page(request: Request) -> Response:
    with db.connect(request.app.state.settings.db_path) as conn:
        files = list_files(conn)
    return render(request, "files.html", section="files", title="Files", files=files)
...
async def api_upload(request: Request) -> dict:
    ...
    def insert() -> dict:
        with db.connect(settings.db_path) as conn:
            return insert_file(conn, file_id, name, size, sha256)
